...
def get_colors_depth(img, ver):
    [h, w, _] = img.shape
    ver[0, :] = np.minimum(np.maximum(ver[0, :], 0), w - 1)  # x
    ver[1, :] = np.minimum(np.maximum(ver[1, :], 0), h - 1)  # y
    circ = ver[0,:] ** 2 + ver[1, :] ** 2
    ver = np.vstack((ver, circ))
    ind = np.round(ver).astype(np.int32)
    print('STARTING')
    print(ind.shape)

    # creates an array of indices, sorted by unique element
    idx_sort = np.argsort(ver[3, :])
    
    # sorts records array so all unique elements are together 
    sorted_ver = ver[:, idx_sort]
    print('few shapes..')
    print(sorted_ver.shape)
    print(sorted_ver[:2, :].shape)
    print(sorted_ver[:2, :5])
    
    # returns the unique values, the index of the first occurrence of a value, and the count for each element
    vals, idx_start, count = np.unique(sorted_ver[:2], return_counts=True, return_index=True, axis=1)
    print('vals shapes: ', vals.shape)
    print('few vals: ') 
    print(vals[:, :5])
    ...
    res_sz = np.asarray([x.size for x in res])
    print('max cluster: ', np.max(res_sz))
    print(ind[:, res[0][:10]])
    ...
    for dup in res:
        ...
        for i in dup:
            ...
            #print(colors[i, :])
            colors[i, :] = 0
            #print('after', colors[i, :])
        colors[champ, :] = img[ind[1, champ], ind[0,champ]] / 255.
        #print('CHAMP: ', colors[champ, :])
    print('Finished')
