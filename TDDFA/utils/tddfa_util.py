...
import numpy as np
import torch
from TDDFA.bfm import bfm, tri
from TDDFA.utils.pose import matrix2angle, P2sRt
...
def recon_dense(param, roi_box, size):
    ...
    print('scaled R:')
    print(R / s)
    print(R_ @ np.transpose(R_))
    print(offset)
    pts3d = R @ (u + w_shp @ alpha_shp + w_exp @ alpha_exp).reshape(3, -1, order='F') + offset
    pts3d = similar_transform(pts3d, roi_box, size)
    pts3d_origin = s * (u + w_shp @ alpha_shp + w_exp @ alpha_exp).reshape(3, -1, order='F') + offset
    pts3d_origin = similar_transform(pts3d_origin, roi_box, size) 
